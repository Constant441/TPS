...
FSettingOption UTPSGameSetting::GetCurrentOption() const
{
    ...
    const auto Option = Options.FindByPredicate([&](const FSettingOption& Opt) {
...
int32 UTPSGameSetting::GetCurrentIndex()
{
    const int32 CurrentValue = GetCurrentValue();
    return Options.IndexOfByPredicate([&](const auto& Opt) {
        return CurrentValue == Opt.Value;
    });
}
